const {
  ...
  const readline = require('readline') // 🌟 Pair code input එකට අවශ්‍යයි
  ...
  // Terminal එකෙන් Input එකක් ගන්න (Pairing Code එක පෙන්වන්න)
  const rl = readline.createInterface({ input: process.stdin, output: process.stdout });
  const question = (text) => new Promise((resolve) => rl.question(text, resolve));

  //===================SESSION-AUTH============================
if (!fs.existsSync(__dirname + '/Elisa-sessions/creds.json')) {
if(!settings.SESSION_ID || settings.SESSION_ID === "YOUR_SESSION_ID") {
    console.log('No Session ID found. Proceeding with Pair Code Login... 🔑');
} else {
    ...
    console.log("Qᴜᴇᴇɴ ᴀʀᴜɴᴅᴀᴛʜɪ Session downloaded ✅")
    })})
}}
...
const port = process.env.PORT || 9090;
  ...
  console.log("Qᴜᴇᴇɴ ᴀʀᴜɴᴅᴀᴛʜɪ Connecting to WhatsApp ⏳️...");
  const { state, saveCreds } = await useMultiFileAuthState(__dirname + '/Elisa-sessions/')
  var { version } = await fetchLatestBaileysVersion()
  
  const conn = makeWASocket({
          logger: P({ level: 'silent' }),
          printQRInTerminal: false, // 👈 QR පෙන්වන එක නැවැත්තුවා
          browser: [ "Ubuntu", "Chrome", "20.0.04" ], // 👈 Pair code වලට අත්‍යවශ්‍යයි
          syncFullHistory: true,
          auth: state,
          version
          })
      
  // 🌟 මෙතනින් තමයි ටර්මිනල් එකේදී Pair Code එක generate වෙන්නේ
  if (!conn.authState.creds.registered) {
      setTimeout(async () => {
          let phoneNumber = await question('\n👑 Qᴜᴇᴇɴ ᴀʀᴜɴᴅᴀᴛʜɪ 👑\nਕරුණාකර ඔබේ WhatsApp අංකය ඇතුළත් කරන්න (රටේ කේතය සමඟ, උදා: 9477xxxxxxx):\n');
          phoneNumber = phoneNumber.replace(/[^0-9]/g, '');
          
          let code = await conn.requestPairingCode(phoneNumber);
          code = code?.match(/.{1,4}/g)?.join("-") || code;
          console.log(`\n👑 Qᴜᴇᴇɴ ᴀʀᴜɴᴅᴀᴛʜɪ LINK CODE: ${code}\n`);
      }, 3000);
  }
  ...
  console.log('Qᴜᴇᴇɴ ᴀʀᴜɴᴅᴀᴛʜɪ Plugins installed successful ✅')
  console.log('Qᴜᴇᴇɴ ᴀʀᴜɴᴅᴀᴛʜɪ CONNECTED SUCCESSFULLY ✅')
  
  let up = `*Hello there Qᴜᴇᴇɴ ᴀʀᴜɴᴅᴀᴛʜɪ User! \ud83d\udc4b\ud83c\udffb* \n\n> Simple , Straight Forward But Loaded With Features \ud83c\udf8a, Meet Qᴜᴇᴇɴ ᴀʀᴜɴᴅᴀᴛʜɪ WhatsApp Bot.\n\n *Thanks for using Qᴜᴇᴇɴ ᴀʀᴜɴᴅᴀᴛʜɪ \ud83d\udea9* \n\n> Join WhatsApp Channel :- \n\n- *YOUR PREFIX:* = ${prefix}\n\n- *BOT MODE:* = ${settings.MODE}\n\n> ©ᴍᴀᴅᴇ ʙʏ Qᴜᴇᴇɴ ᴀʀᴜɴᴅᴀᴛʜɪ \ud83d\udda4`;
...
if (!isReact && settings.CUSTOM_REACT === 'true') {
  ...
  app.get("/", (req, res) => {
  res.send("Qᴜᴇᴇɴ ᴀʀᴜɴᴅᴀᴛʜɪ MULTIDEVICE WHATSAPP BOT STARTED ✅");
  });
